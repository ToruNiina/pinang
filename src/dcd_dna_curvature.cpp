    ...
    */
    ...
    for (int h = begin_frame; h < end_frame; h+=step) {
        ...
        for (i = 0; i < len1; i++) {
            ...
            backbone1_nodes.push_back(confm.atom(j));
        }
        for (i = 0; i < len2; i++) {
            j = phosphate_strand2_idx[i];
            backbone2_nodes.push_back(confm.atom(j));
        }
        len1 = base_strand1_idx.size();
        len2 = base_strand2_idx.size();
        for (i = 0; i < len1; i++) {
            j = base_strand1_idx[i];
            base_positions1.push_back(confm.atom(j));
        }
        for (i = 0; i < len2; i++) {
            j = base_strand2_idx[i];
            base_positions2.push_back(confm.atom(j));
        }
