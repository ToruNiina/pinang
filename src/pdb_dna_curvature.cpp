...
void gen_spline_fit(const std::vector<Eigen::Vector3d>&, int N,
...
int main(int argc, char *argv[])
{
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  */
  std::vector<Eigen::Vector3d> backbone1_dots;  // Interpolation points;
  std::vector<Eigen::Vector3d> backbone1_nodes; // P atoms in the 1st backbone;
  std::vector<Eigen::Vector3d> backbone1_tangents; // Tangents at "P" atoms;
  std::vector<Eigen::Vector3d> backbone1_normals; // Normal vectors at "P" atoms;

  std::vector<Eigen::Vector3d> backbone2_dots;
  std::vector<Eigen::Vector3d> backbone2_nodes; // P atoms in the 2nd backbone;
  std::vector<Eigen::Vector3d> backbone2_tangents;
  std::vector<Eigen::Vector3d> backbone2_normals; // Normal vectors at "P" atoms;

  std::vector< std::vector<Eigen::Vector3d> > genline1_lines; // Generating Lines!
  std::vector< std::vector<Eigen::Vector3d> > genline2_lines;
  std::vector< std::vector<Eigen::Vector3d> > genline1_line_tangents;
  std::vector< std::vector<Eigen::Vector3d> > genline2_line_tangents;

  std::vector<Eigen::Vector3d> base_positions1;
  std::vector<Eigen::Vector3d> base_positions2;

  std::vector<Eigen::Vector3d> axis_nodes;
  std::vector<Eigen::Vector3d> axis_directions; // Axis direction vectors;
  ...
  */
  ...
  for (i = 0; i < 10; i++) {
    std::vector<Eigen::Vector3d> genline1_dots;  // Interpolation points;
    std::vector<Eigen::Vector3d> genline1_nodes; // P atoms in the 1st backbone;
    std::vector<Eigen::Vector3d> genline1_tangents; // Tangents at "P" atoms;
    std::vector<Eigen::Vector3d> genline2_dots;
    std::vector<Eigen::Vector3d> genline2_nodes; // P atoms in the 2nd backbone;
    std::vector<Eigen::Vector3d> genline2_tangents;
  ...
  */
  ...
  for (i = 0; i < int(backbone1_nodes.size()); i++) {
    int m = i / 10;
    int n = i % 10;
    Eigen::Vector3d t1 = backbone1_tangents[i];
    Eigen::Vector3d t2 = genline1_line_tangents[n][m];
    Eigen::Vector3d nm = t2.cross(t1);
    Eigen::Vector3d n0 = nm / nm.norm();
    backbone1_normals.push_back(n0);
  }
  for (i = 0; i < int(backbone2_nodes.size()); i++) {
    int m = i / 10;
    int n = i % 10;
    Eigen::Vector3d t1 = backbone2_tangents[i];
    Eigen::Vector3d t2 = genline2_line_tangents[n][m];
    Eigen::Vector3d nm = t2.cross(t1);
    Eigen::Vector3d n0 = nm / nm.norm();
  ...
  for (i = 0; i < int(backbone1_normals.size()); i++) {
    Eigen::Vector3d t1 = backbone1_normals[i];
    Eigen::Vector3d t2 = genline1_line_tangents[i%10][i/10];
    Eigen::Vector3d t3 = t1.cross(t2);
    Eigen::Vector3d nm;       // normal vector of the fake plane
    ...
    for (j = 0; j < int(base_positions2.size()); j++) {
      _d_ = pinang::MyVec::vec_distance(base_positions1[i], base_positions2[j]);
      if (_d_ < d_m) {
        d_m = _d_;
        cog_base_id = j;
      }
    ...
    for (theta = - angle_lim ; theta <= angle_lim; theta += pi_over_36) {
      ...
      for (alpha = - angle_lim2 ; alpha <= angle_lim2; alpha += pi_over_60) {
        v_tmp = nm + t1 * tan(alpha);
        double l = v_tmp.norm();
        nm = v_tmp / l;
        // STEP 1: calculate the plane! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // Plane: a x + b y + c z = _d0
        double _d0 = nm.dot(backbone1_nodes[i]); // param in plane function!
        ...
        for (j = 0; j < int(genline1_lines.size()); j++) {
          ...
          for (int k = k_min; k < k_max; k++) {
            _d = genline1_lines[j][k].dot(nm) - _d0;
        ...
        for (j = 0; j < int(genline2_lines.size()); j++) {
          ...
          int k_max = std::min(s + 50, int(genline1_lines[j].size()));
          for (int k = k_min; k < k_max; k++) {
            _d = genline2_lines[j][k].dot(nm) - _d0;
            double d_tmp = _d > 0 ? _d : - _d;
            if (d_tmp <= _dm) {
              _dm = d_tmp;
        ...
        for (j = 0; j < int(intersects.size()); j++) {
          d_tmp = pinang::MyVec::vec_distance(com, intersects[j]);
          if (d_tmp > d_max)
            d_max = d_tmp;
        }
  ...
  for (i = 4; i < int(axis_nodes.size())-4; i++) {
    j = i - 3;
    Eigen::Vector3d v1 = axis_nodes[i + 1] - axis_nodes[i - 1];
    Eigen::Vector3d v2 = axis_nodes[i + 2] - axis_nodes[i - 2];
    Eigen::Vector3d v3 = axis_nodes[i + 3] - axis_nodes[i - 3];
    Eigen::Vector3d dir1(0,0,0);
    Eigen::Vector3d dir2(0,0,0);
    Eigen::Vector3d dir3(0,0,0);
    ...
    if ( j >=1 && j < int(axis_directions.size()) - 1) {
      ...
      k1 = pinang::MyVec::vec_angle(dir1, dir2) / dist1;
      if (dist1 > 3) {
        k_sum += k1;
        k_count += 1;
        f_only_k1 = 1;
    ...
    if ( j >=2 && j < int(axis_directions.size()) - 2) {
      ...
      k2 = pinang::MyVec::vec_angle(dir1, dir2) / dist2;
      if (dist2 > 3) {
        k_sum += k2;
        k_count += 1;
        f_only_k1 = 0;
    ...
    if ( j >=3 && j < int(axis_directions.size()) - 3) {
      ...
      k3 = pinang::MyVec::vec_angle(dir1, dir2) / dist3;
      if (dist3 > 3) {
        k_sum += k3;
        k_count += 1;
        f_only_k1 = 0;
  ...
  */
  ...
  for (i = 0; i < int(axis_directions.size())-1; i++) {
    ...
    // Plane: a x + b y + c z = _d0
    j = i + 4;
    Eigen::Vector3d nm = axis_directions[i];
    Eigen::Vector3d _O = axis_nodes[j];
    double _d0 = nm.dot(_O); // param in plane function!
    double _d = 0;
    ...
    int kb2 = 0;
    for (int k = k_min; k < k_max; k++) {
      _d = backbone1_dots[k].dot(nm) - _d0;
      double d_tmp = _d > 0 ? _d : - _d;
      if (d_tmp <= _dm) {
        _dm = d_tmp;
    ...
    for (int k = k_min; k < k_max; k++) {
      _d = backbone2_dots[k].dot(nm) - _d0;
      double d_tmp = _d > 0 ? _d : - _d;
      if (d_tmp <= _dm) {
        _dm = d_tmp;
    ...
    Eigen::Vector3d v1;
    Eigen::Vector3d v2;
    Eigen::Vector3d groove_D1;
    Eigen::Vector3d nm_plane;

    Eigen::Vector3d I1_0;   // backbone 1
    Eigen::Vector3d I3_0;   // backbone 1

    Eigen::Vector3d I2_0;   // backbone 2
    Eigen::Vector3d I4_0;   // backbone 2
    ...
    vtmp = v1 + v2;
    d = vtmp.norm();
    groove_D1 = nm.cross(v1);    // ****************************** OMG~
    if (vtmp.norm() > 0.05) {
      if (vtmp.dot(groove_D1) > 0)
        groove_D1 = vtmp / d;
      else
        groove_D1 = vtmp / (-d);
    }
    ...
    Eigen::Vector3d t3 = groove_D1.cross(nm); // plane normal vector
    double theta = 0;
    double angle_lim = pinang::k_pi / 2;   // 90 degree;
    for (theta = - angle_lim; theta <= 0; theta += pi_over_60) {
      Eigen::Vector3d I1;   // backbone 1
      Eigen::Vector3d I2;   // backbone 2
      ...
      d = nm_plane.norm();
      nm_plane = nm_plane / d;
      _d0 = nm_plane.dot(_O); // param in plane function!
      ...
      for (k = k_min; k < k_max; k++) {
        _d = backbone1_dots[k].dot(nm_plane) - _d0;
        double d_tmp = _d > 0 ? _d : - _d;
        if (d_tmp <= _dm) {
          _dm = d_tmp;
          I1 = backbone1_dots[k];
      ...
      for (k = k_min; k < k_max; k++) {
        _d = backbone2_dots[k].dot(nm_plane) - _d0;
        double d_tmp = _d > 0 ? _d : - _d;
        if (d_tmp <= _dm) {
          _dm = d_tmp;
          I2 = backbone2_dots[k];
    ...
    double ang1 = acos(nm.dot(backbone1_tangents[kb1/10]));
    double ang2 = acos(nm.dot(backbone2_tangents[kb2/10]));
    if (ang1 > 1.570795)
    ...
    for (theta = 0; theta <= angle_lim; theta += pi_over_60) {
      Eigen::Vector3d I3;   // backbone 1
      Eigen::Vector3d I4;   // backbone 2
...
//  |___/          |_____|  |_|                 |_____|
*/
void gen_spline_fit(const std::vector<Eigen::Vector3d>& nodes, int N,
                    std::vector<Eigen::Vector3d>& dots,
                    std::vector<Eigen::Vector3d>& tangents)
{
  ...
  t1 = tangents[1].dot(tangents[2]) * 2;
  v1 = tangents[1] * t1;
  tangents[0] = v1 - tangents[2]; // fixed tangent 0;

  t1 = tangents[len1-3].dot(tangents[len1-2]) * 2;
  ...
  for (int i = 0; i < len1 - 1; i++) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      r2 = r * r;
      r3 = r * r * r;
      b_r = p_i + v1 * ( d * ( r - 2*r2 + r3))
            + t_tmp * ( 3*r2 - 2*r3 ) + v2 * (d * (r3 - r2));
      dots.push_back(b_r);
    }
  }
  p_i = nodes[len1-1];
