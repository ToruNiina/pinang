...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  std::vector<Eigen::Vector3d> c_alpha_coors;
    ...
    for (j = 0; j < mdl_size; ++j)
      if (mdl0.get_chain(j).get_chain_ID() == chain_id[i]) {
        ...
        for (int k = 1; k < len1; k++) {
          c_alpha_coors.push_back(chain_tmp.get_residue(k).get_C_alpha().get_coordinates());
        }
        chain_tmp.reset();
      }
  ...
  Eigen::Vector3d com_ca(0,0,0);
  Eigen::Vector3d com_all(0,0,0);
  for (i = 0; i < len_ca_coors; ++i) {
    com_all = com_all + c_alpha_coors[i];
  }
  com_ca = com_all / len_ca_coors;

  double dist_square_sum = 0;
  for (i = 0; i < len_ca_coors; ++i) {
    Eigen::Vector3d v_dist = c_alpha_coors[i] - com_ca;
    double sqr_dist = v_dist.squaredNorm();
    // std::cout << i << "  " << sqrt(sqr_dist) << std::endl;
