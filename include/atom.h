...
class Atom
{
 public:
  ...
  inline const Eigen::Vector3d& get_coordinates() const;
  inline void set_coords(const Eigen::Vector3d& coors);
  ...
  inline void set_charge(double s);
...
inline const Eigen::Vector3d& Atom::get_coordinates() const
{
  return coordinate_;
}
inline void Atom::set_coords(const Eigen::Vector3d& coors)
{
  coordinate_ = coors;
}
...
inline void Atom::set_charge(double s)
{
  charge_ = s;
}
...
inline std::ostream& operator<<(std::ostream& o, const Atom& a)
{
  ...
  if (a.atom_flag() == "ATOM  " || a.atom_flag() == "HETATM")
  {
    o << std::setw(6) << a.get_atom_flag()
      ...
      << std::setw(8) << tmp_coors(0)
      << std::setw(8) << tmp_coors(1)
      << std::setw(8) << tmp_coors(2)
...
std::istream& operator>>(std::istream& i, Atom& a)
{
  ...
  if (a.atom_flag() == "ATOM  " || a.atom_flag() == "HETATM")
  {
    ...
    tmp_sstr.str(pdb_line.substr(78,1));
    tmp_sstr >> tmp_ui;
...
{
  Eigen::Vector3d v3 = a1.get_coordinates() - a2.get_coordinates();
  return v3.norm();
}
